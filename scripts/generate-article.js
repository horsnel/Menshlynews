 ...
 * 
 * Usage:
 *   node scripts/generate-article.js "Article Title" "Category" "Excerpt"
 *   node scripts/generate-article.js "AI Drop Servicing Business" "Side Hustles" "How to build a $6K/month drop servicing business"
 * 
 * Or interactive mode:
...
- Use markdown tables with | separators (they render properly)
...
async function main() {
  ...
  if (args.length >= 3) {
    ...
    console.log('================================\n');
    console.log('Usage: node scripts/generate-article.js "Title" "Category" "Excerpt"');
    console.log(`Valid categories: ${VALID_CATEGORIES.join(', ')}\n`);
    
    // Generate a random article topic suggestion
    const topics = [
    ...
    console.log(`   node scripts/generate-article.js "${random.title}" "${random.category}" "${random.excerpt}"\n`);
    process.exit(1);
  }
  ...
  const content = await generateContent(title, category, excerpt);
  const wordCount = content.split(/\s+/).length;
  console.log(`   ✅ Content generated: ${wordCount} words`);
  
  if (wordCount < 1800) {
    console.log('   ⚠️  Content is under 2000 words, regenerating...');
    ...
    if (retryWords > wordCount) {
      console.log(`   ✅ Retry better: ${retryWords} words`);
    }
  }
  
  // Step 2: Generate hero image
  console.log('\n🎨 Step 2: Generating hero image...');
  const imageFilename = `article-generated-${Date.now()}.png`;
  ...
  console.log(`   Image: /images/${imageFilename}`);
  console.log(`   Slug: ${slug}`);
  console.log('\n   Next steps:');
  console.log('   1. Review the article in data.ts');
...
function buildImagePrompt(title: string, category: string): string {
  const prompts: Record<string, string> = {
    ...
    'Saving': 'Smart saving and budgeting workspace, laptop with financial planning app, organized desk with calculator and piggy bank, modern clean style, editorial photography',
...
function extractTags(title: string, category: string): string[] {
  ...
  const stopWords = new Set(['a', 'an', 'the', 'for', 'and', 'or', 'in', 'on', 'to', 'with', 'using', 'how', 'your', 'from', 'build', 'start']);
...
function insertPostIntoDataTs(post: typeof globalThis): void {
